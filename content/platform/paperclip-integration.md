...
|---|---|---|---|
| Clarity Steward | maintenance | every 4h | Runs /se:self-improve, validates YAML, compresses bloated expertise files |
| Wiki Curator | maintenance | every 30m | Checks raw/ for files, runs /se:wiki-ingest and /se:wiki-lint |
...
Claude Code child process (with PAPERCLIP_* env vars)
    |
    v  [agent reads expertise.yaml, runs /se:* commands]
